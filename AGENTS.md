...
- `docs/COMPANY.md`는 `0.1.3-candidate`, `docs/WORK_ORDERS.md`의 `SB WO-003`은 `검수` 상태입니다. 독립 Checker와 CEO 판정 전에는 완료·정본 승격을 선언하지 않습니다.
- 코드 통합 기준은 `feature/webtoon-studio`의 `f90b8751`입니다. local main skill 이력과 PPT 스타일 자산은 보존됐지만 외부 push·deploy는 하지 않았습니다.
