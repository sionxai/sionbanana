...
- **B(화면, Codex 위임 1회·왕복 0):** 새 모션 대화상자 '영상' 탭 — `GET /api/video` 목록(최근순 50) → 선택 시 프로브 호출 → 접촉 시트·검출 결과·제안 프레임·파생 fps 표시, 모드/프레임 수/구간/재생 fps 입력, 영상 요청에 `grid` 미전송, 응답 역전·언마운트 방어. `GET /api/video`(최대 100, POST 불변). 편집 화면 배지 "영상 프레임 N장 · 검증된 배치" + 출처(영상 id·원본/재생 fps·모드·구간).
...
- **(추가 재계측, 대표 "방금 코덱스 업데이트하니 떴다")** ChatGPT 앱 26.903(07:06 갱신)에 내장된 codex와 npm CLI 모두 0.153.4 동일, 2.5·sunburst·flare 문자열 없음. 오늘 롤아웃·앱 로그·캐시에 2.5 요청 흔적 0. 브리지 에코 프로브 4요청 추가(`originator` codex_cli_rs / codex_app_server / Codex Desktop + installation-id, body gpt-6-astra, tool.model 2.5-flare) — **전부 `gpt-image-2-codex`.** 결론: 앱 UI에 보이는 것은 서버가 내려주는 목록이며, 우리 경로(`codex/responses` image_generation)에서는 모델·헤더로 2.5를 선택하거나 관측할 수 없다. 별칭 뒤 백엔드 교체 여부는 이름으로 판별 불가(속도·품질 표본만 가능). 다음 단서: 대표가 Codex 앱에서 이미지 1장을 생성하면 그 세션 롤아웃(`~/.codex/sessions`)에서 앱이 쓴 도구 사양·모델 id를 확인할 수 있다.

### SB WO-024 — 모션 정규화의 이펙트 오인 수정: X 보존 모드·시퀀스 피사체 판정·정렬 게이트 (2026-09-10, 대표 승인)

- **발단:** TEF 세션 의뢰(대표 전달). K2 전차 사격 모션(영상 소스, `motion-1789012553018-…`)에서 8프레임 중 1프레임이 428px 밀림. 원인은 `analyzeFrame`이 최대 연결 성분을 피사체로 확정하고 `normalizeFrames`가 그 발 접점에 맞춰 내용을 이동시키는 구조.
- **CEO 재현(실제 매트 허용치 52):** 원본 프레임 19에서 포연이 차체와 **분리된 성분**으로 26,239px, 차체 25,709px(2% 차이) → 발 접점 652 vs 나머지 226~229. 연결 경로는 이 영상에서 미발생. 지속성(존재 비율 ≥0.5 픽셀) 기준 피사체로 재계산하면 8프레임 223~229. 기본값 불일치(엔진 foot / API·스키마 centroid)와 게이트가 정렬을 안 본다는 지적 사실.
- **계획(승인):** A `normalizePivotX:"preserve"`(프레임 이동 없음 + 프레임별 피사체 발 접점 중앙값을 공유 pivot으로; 영상 소스 신규 기본값 preserve, 엔진 기본값 centroid로 통일) · B 시퀀스 지속성 피사체 판정(정렬된 셀 좌표계인 grid/영상만, trim은 전체 전경 유지) · C 프레임별 앵커 편차 `alignment` 기록 + `frame-N-alignment-outlier` 승인형 게이트(임계값은 기존 프로젝트 전수 계측으로 확정). 기존 산출물 소급 재생성 없음, atlas·ZIP 구성 불변. Codex 2회(A+B / C).
- **완료 판정(의뢰 조건):** `n9n84fz1esjmtuzoa26`를 `oneshot 13~59, count 8, object`로 재생성 시 차체 bbox 좌단 편차 5px 이내(수치 보고) · 대조군 주행 루프 결과 불변 · preview.gif 실제 재생 확인 · 전체 스위트 통과.

### SB WO-025 — 모션 흐름에서 원본 영상 재생 + 선택 프레임 시각 안내 (2026-09-10, 대표 승인)

- **발단:** 대표 지적 "영상을 만들고 프레임을 뽑는데 원본 영상을 볼 루트가 없다". CEO 확인 — `/api/videos/<id>` 스트리밍 라우트와 히스토리 뷰의 영상 항목은 있으나, 새 모션 "영상" 탭은 접촉 시트만, 편집 화면은 출처 id만 보여준다.
- **계획(약식·소):** "영상" 탭의 선택 영상 아래 인라인 `<video>` 재생, 편집 화면의 출처 배지 아래 접이식 "원본 영상"(재생 + 선택 프레임 번호·시각 칩, 클릭 시 해당 시각으로 이동). 변경 파일 `motion-create-dialog.tsx`·`motion-editor-panel.tsx` 2개, API 변경 없음(Range 미지원은 로컬 수 MB라 전부 받은 뒤 탐색 가능 — 한계로 기록). WO-024 뒤 같은 워크트리에서 진행.
- **WO-024 결과(2026-09-10):** A+B 커밋 `(워크트리) 1라운드`, C 커밋 `2라운드`, 병합 `8cb64dbf`(WO-025와 함께)·`33f0d743`(보정). Codex 2회 위임·왕복 0·CEO 직접 수정 0줄(preload 보정 2줄은 WO-025). Codex는 키체인 크래시로 검증 미실행 → CEO 실행: 워크트리 313→316, 병합본 **324/324**, tsc·lint 통과.
  - **B 규칙 확정:** 계획의 지속성(픽셀 존재 비율) 규칙은 포연이 8프레임 중 4프레임에 남는 v4에서 centroid가 166px 흔들려 기각. 플랜 B였던 **"프레임 간 피사체 bbox 중앙값과 IoU 최대 성분"**(2회 반복)을 채택 — 전수 16건에서 결함 2건만 변경(449→3px, 424→4px), 14건은 선택 성분·수치 완전 동일.
  - **C 임계값:** `max(round(cellWidth×0.15), 24)`. 전수 계측 근거: 정상 최대 편차 45px(여전사 검 베기 lunge, 셀 640 → 7%), 결함 424·449px(셀 848 → 50%+). 오탐 0 / 검출 2/2. 레거시·auto 프로젝트는 `alignment: null`(새 사유 안 붙음).
  - **라이브(실서버 3002, 생성 호출 0):** `n9n84fz1esjmtuzoa26`을 `oneshot 13~59, count 8, object`로 재생성 — 기본 매트: 차체 bbox 좌단 [4,2,4,6,6,6,6,6] **편차 4px**, 허용치 52·부드러움 3: [4,2,6,6,6,6,6,6] **편차 4px**(기준 5px). 두 건 모두 `alignment.outliers []`, 공유 pivot 1점, 내보내기 200(차단 없음), atlas 8프레임 pivot 동일(0.2339,0.9469). preview.gif 8프레임을 타일로 펼쳐 **눈으로 확인**: 차체 고정, 포연만 이동. 대조군 주행 루프를 저장 설정(foot/pin/none·허용치 45·구간 54~62)으로 재생성 → 기존 derived 프레임과 **픽셀 차이 0**, trim.x 동일.
  - **판정: PASS · 완료.** 한계: 포연이 차체와 연결돼 한 성분이 되는 경로는 이번 영상에서 미발생이라 미검증(게이트가 잡는다). 기존 결함 프로젝트는 소급 재생성하지 않았다(TEF 배포 중) — 재빌드(매트 변경 등) 시에만 새 규칙 적용. 워크트리 `sb-wo-024-alignment` 병합 완료·clean 유지.
...
- **7단계 원격 push 완료(2026-09-18):** 대표 승인 후 이 세션의 push는 Claude Code 자동 권한 검사에 막혀 **대표가 직접 실행**했다. 원격 `main` `bfe49df8` → `294675ce`, 원격 `feature/webtoon-studio` `5af8de94` → `294675ce`(빨리감기, force 없음). 사전 점검: 저장소는 **공개**(14일 조회 0·별·포크·구독 0·유입 없음, 클론 6곳 8회는 조회 없는 자동 수집으로 추정), 올린 커밋의 토큰·키·계정 ID·비밀번호 형태 스캔 0건, 맥 홈 경로가 4개 파일에 포함. 대표 판단 "보는 사람 없다"로 공개 그대로 진행. 남은 미결은 4단계 작업본 전환과 워크트리 정리(friendly-pasteur 삭제 가능 · arduino 무시 파일 181M 대조 필요 · key-visual-work 세션 종료 후).
