...
★★**단, 뺄 수 있는 것은 장식과 조연이지 신체의 일부가 아니다.** 이 규칙을 오해해 「여성의 다리가 세 개로 나온다 → 다리를 하나만 프레임에 걸치게 한다」로 적용했다가, 개수 오류는 사라졌지만 **몸통 없이 다리 하나만 잘려 들어온 더 기괴한 그림**이 나왔다. 해부학적 개수 오류는 프레임에서 신체를 잘라 해결하지 말고 **구도 자체를 바꿔라**(다음 절 참조).
...
## 이미지 프롬프트 작성법 — 43회 실패 뒤 재현으로 검증한 원칙 (이루 세션)

한 장을 완성하는 데 **43회**를 쓴 뒤, 아래 원칙을 적용하자 같은 난이도의 장면이 **2회**와 **3회**로 끝났다. 두 번 재현됐으므로 규칙으로 승격한다.
...
### 23. ★크기는 프레임 안의 **변하지 않는 물체**로 지정한다

수치 비율(`컵 높이의 90%`)은 **세 판 연속 무시됐다.** 모델은 주제성을 유지하려고 주인공을 크게 만드는 경향이 있다.

★ **기준물 선택이 설계다.** 「강아지가 컵보다 작다」로 쓰면 **컵이 자가 되어 정상 크기로 고정된다** — 소품까지 작게 만들려던 의도와 정반대다. 이 오류는 검수로 잡히지 않는다(다음 절).

**기준물은 그 장면에서 크기가 변하지 않는 것으로 고른다:** 사람의 구두, 좌석 등받이, 금속 O링, 지퍼 손잡이.

> `앉은 강아지의 키는 좌석 등받이 높이의 6분의 1이며, 여성의 구두 한 짝이 강아지보다 길다.`
> `작아진 강아지의 머리 폭이 은색 O링 지름의 세 배를 넘지 않는다.`
...
## 프롬프트가 부른 실패들 — 키링 시리즈 9회차에서 원인 분리 (이루 키링 세션)

8장짜리 시리즈를 회차마다 변인을 좁혀 가며 돌렸고, 실패마다 **내가 쓴 문장이 범인**이었다. 아래는 원인이 분리된 것만 적는다.
...
### 31. ★크기 기준물은 「크기가 자명한 것」 + 절대 치수 병기 (§23 보강)

불변 물체라도 **크기가 모호하면 자 역할을 못 한다.** 실측: 무릎뼈 폭·가방 어깨끈 폭을 기준으로 쓴 컷은 전부 크기가 무너졌고, 손가락 마디·엄지·손톱을 쓴 컷은 전부 맞았다.

- 기준물은 **누구나 크기를 아는 것**으로: 사람 손가락 마디, 엄지, 손톱, 열쇠, 이어폰 케이스
...
- 시트가 못 고정하는 것: **어디에 있는가** — 위치는 시점 모순(§32)의 문제라 시트로 안 풀린다
- 다패널 시트는 통째로 주지 말고 **목적에 맞는 패널만 크롭해** 별도 참조로 등록하라. 3면 시트의 측면 컷이 오히려 옆구리 파우치로 오독됐고, 후면 컷만 잘라 주자 해결됐다
- ★시트를 산문으로 옮길 때 **시트를 다시 열어 대조하라.** 시트에 없는 「귀가 서 있다」, 시트와 다른 「이마의 점」(실제는 세로 줄)을 내 기억으로 써 넣어 캐릭터가 어긋났다. §22 자산 대장의 프롬프트판이다
...
### 35. ★착용물에는 수평·고정 조항을 명시하라

배낭이 등 중심선에서 옆으로 미끄러져 삐딱하게 나왔다. 착용물은 놔두면 **중력과 자세를 따라 흘러내린 것처럼** 그려진다. 고정 조항:
...
1. 크기 — [불변 기준물] 대비로. 결합된 소품은 비례로 함께 묶는다.
