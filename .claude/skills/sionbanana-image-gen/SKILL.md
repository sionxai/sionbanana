...
## 썸네일 — 스토리보드 컷과 요구가 다르다

### 20. 한글은 큰 전경 글자면 잘 나온다

배경 간판이 유사 한글로 뭉개지는 것을 보고 "제목은 합성해야 한다"고 판단했으나 **틀렸다.** 화면 폭의 절반을 차지하는 제목 글자는 「HOPO 왜군침공」·「1987」·「외계인침공은 잊어라」가 모두 또렷하게 나왔다.

**갈리는 지점은 크기다.** 작은 배경 간판은 뭉개지고 큰 전경 제목은 나온다. 다만 화면에 넣을 글자는 **명시한 것만** 나오게 잠가라:

> `★★이 두 줄 외에 어떤 글자도 넣지 말 것 — 부제·간판 글씨·자막·로고·워터마크 금지.`

### 21. ★글자 크기는 "화면 폭의 %"가 아니라 프레임 내 비율로 줘라

`화면 폭의 12%`는 안 잡힌다. **프레임 안에서 직접 비교되는 값**으로 바꾸면 한 번에 나온다:

> `★1행 한 글자의 높이가 ★화면 세로의 5분의 1이다.`
> `★1행 전체가 ★화면 가로의 55~60%를 차지한다.`
> `★작고 얌전한 자막처럼 그리지 말 것 — 포스터 제목처럼 크다.`

§10의 "결과가 아니라 물리"를 조판에 적용한 것이다.

### 22. 판정 기준은 원본이 아니라 ★실제 표시 크기다

썸네일을 원본 해상도로 보고 판단하지 마라. **유튜브 실제 표시 폭으로 줄여서 보는 것이 유일한 판정이다.**
...
실측 결과:
- 글자 높이 **8%**: 320px에서 제목은 읽히나 **부제는 뭉개짐**, 168px에서는 제목도 위태
- 글자 높이 **12%** + **두 줄**: 320·168 양쪽에서 읽힘

**세 줄은 쓰지 마라.** 세 번째 줄은 어느 크기에서도 안 읽히면서 공간만 먹는다. 그 줄을 빼고 제목을 키우면 전체가 산다. 빼야 할 문구가 아깝다면 **영상 제목으로 옮겨라.**

배경 밝기도 크기만큼 중요하다 — 밝은 하늘에 얹은 제목은 아무리 키워도 168px에서 묻힌다. **글자는 어두운 영역에 앉혀라.**
